...
def transition_native_child(
    ...
    *,
    ...
    with SessionLocal() as db:
        ...
        if row is None:
            return None
        if row.state in _NATIVE_CHILD_TERMINAL_STATES and state in _NATIVE_CHILD_ACTIVE_STATES:
            return _native_child_row(row)
        row.state = state
        row.updated_at = now
        ...
        if error_summary is not None:
            row.error_summary = error_summary[:1024]
        if state in _NATIVE_CHILD_TERMINAL_STATES and row.settled_at is None:
            row.settled_at = now
        db.commit()
        db.refresh(row)
        return _native_child_row(row)
    ...
    *,
    ...
    with SessionLocal() as db:
        ...
        for row in rows:
            ...
            row.error_summary = "child lease expired without a durable result receipt"
            row.updated_at = now
            row.settled_at = now
        db.commit()
...
def delete_terminals_by_session(tmux_session: str) -> int:
    ...
    with SessionLocal() as db:
        terminal_ids = [
            cast(str, row[0])
            for row in db.query(TerminalModel.id).filter(TerminalModel.tmux_session == tmux_session).all()
        ]
